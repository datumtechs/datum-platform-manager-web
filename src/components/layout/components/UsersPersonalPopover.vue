<template>
  <el-popover @show="popoverShow = true" @hide="popoverShow = false" placement="bottom-end"
    popper-class="users-el-popover" :width="180" trigger="hover">
    <template #reference>
      <div class="flex items-center" v-if="store.getToken">
        ...
        <span>{{ useAddressDisplay(store.userName) }}</span>
      </div>
    </template>
    <ul class="pl-0px">
      <li @click="toLink('account')"
...
import { useAddressDisplay } from '@/hooks'
const { t } = useI18n()
