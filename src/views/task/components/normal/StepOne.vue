<template>
  <el-form v-bind="$attrs" :label-position="'top'" :rules="rules" :ref="(el: any) => formRef = el" :model="form"
    ...
    <el-form-item v-if="algList.some((item) => item.id == 1000)" :label="`${$t('task.stepOneSelectAlgorithmTitle')}:`"
      prop="algorithm">
    ...
    <template v-if="form.calculationType == 2">
      ...
      <el-form-item v-if="form.calculationType == 2" :label="`${$t('task.stepOneSelectProcedureTitle')}:`"
        prop="procedure">
